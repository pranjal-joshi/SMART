...
class smartNode(TornadoWebSocketClient):
	...
	def received_message(self, msg):
		...
		msg = json.loads(msg)
		self.printRequestInfoClient(msg)
	...
	def printRequestInfoClient(self,msg):
		if DEBUG:
			try:
				print "Name: \t\t" + msg['name']
				print "Type: \t\t" + msg['type']
	...
	def printErrorResponse(self,msg):
		try:
			print "Name: \t\t" + msg['name']
			print "Type: \t\t" + msg['type']
	...
	def pirFall(self):
		global PIR_TIMEOUT_THREAD
		PIR_TIMEOUT_THREAD = threading.Timer(PIR_TIMEOUT, self.presenceReq, 0).start()
		...
		if(POT_FAN_TIME >= APP_FAN_TIME):		# check last recent update is from App or Pot by checking update time stamp.
			if(SPEED > 0):
				...
				if SPEED == 1:
					...
					pwmDealy = 0.01
				threading.Timer(pwmDealy, self.stopPWM).start()
			else:
				self.stopPWM()
		else:
			if(APP_SPEED > 0):
